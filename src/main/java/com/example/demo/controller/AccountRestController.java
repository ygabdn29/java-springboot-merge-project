...
public class AccountRestController {
    ...
    public ResponseEntity<Object> register(@RequestBody RegistrationDTO registrationDTO) {
        try {
            Employee employee = new Employee(null, registrationDTO.getName(), registrationDTO.getEmail(),
                    registrationDTO.getBirthDate(), registrationDTO.getAddress(), registrationDTO.getPhone());
            employeeService.save(employee);

            Role defaultRole = roleService.getRoleWithLowestLevel();
            String guid = UUID.randomUUID().toString();
            User user = new User(registrationDTO.getPassword(), registrationDTO.getUsername(), defaultRole, false, null,
                    employee);
    ...
    public ResponseEntity<Object> save(@RequestHeader("X-token-account-key") String token,
    ...
    public ResponseEntity<Object> validatePassword(@RequestBody String password) {
        if (userService.isValidPassword(password)) {
            ...
            return Utils.generateResponseEntity(HttpStatus.OK,
                    "Password tidak valid. Password harus memiliki minimal 8 karakter, mengandung huruf besar, huruf kecil, dan angka.");
        }
    }
